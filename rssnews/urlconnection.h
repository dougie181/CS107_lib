...
/**
 * Function: URLInitialise
 * -----------------------
 * Because these libraries use CURL and I wanted to make sure
 * they can work in a multi-threaded environment, the URLInitialise()
 * should be called in the calling thread before calling URLConnectionNewA
 * This will performa globabl initiatlisation process of the curl libraries
 * Forgetting to do so will cause some strange behaviours :-)
 */
 ...
 * Function: URLCleanup
 * --------------------
 * Similiar to URLInitiatise, this must be called at the end of the program
 * and if in a MT threaded environment, when all threads have been completed.
 */
